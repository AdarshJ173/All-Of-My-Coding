...
int main() {
    ...
    Node* root1 = new Node(1); // Create root node with value 1
    root1->left = new Node(2); // Create left child node with value 2
    root1->right = new Node(3); // Create right child node with value 3
    root1->left->left = new Node(4); // Create left-left grandchild node with value 4
    root1->left->right = new Node(5); // Create left-right grandchild node with value 5

    // Check if the tree is balanced
    if (isBalancedTree(root1)) {
...
The time complexity of the calculateHeight function is O(n), where n is the number of nodes in the tree. This is because the function recursively traverses the tree, visiting each node once.

The time complexity of the isBalancedTree function is O(n), where n is the number of nodes in the tree. This is because the function recursively traverses the tree, visiting each node once.

The overall time complexity of the program is O(n), where n is the number of nodes in the tree.

Space Complexity Analysis:
The space complexity of the program is O(h), where h is the height of the tree. This is because the maximum depth of the recursive call stack is equal to the height of the tree.

In the worst case, the tree is completely unbalanced, and the height of the tree is equal to the number of nodes (n). Therefore, the space complexity is O(n) in the worst case.
*/